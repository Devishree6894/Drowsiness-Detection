...
def get_db():
    ...
    if db is None:
        db = g._database = sqlite3.connect('driver_status3.db', check_same_thread=False)
    return db
...
def insert_status(state):
    cursor = get_cursor()
    cursor.execute('''INSERT INTO driver_status (state) VALUES (?)''', (state,))
    db = get_db()
    db.commit()
...
def status():
    ...
    timestamps = [datetime.strptime(row[1], "%Y-%m-%d %H:%M:%S") for row in status_data]
    ...
    numeric_states = [status_mapping[state] for state in states]
    
    # Create a color map for the numeric values
    ...
    # Determine the desired number of timestamps to be displayed on the x-axis
    num_timestamps = 10  # Adjust as needed
    
    # Subsample the timestamps to evenly cover the time range
    subsampled_timestamps = [min_timestamp + i * time_range / (num_timestamps - 1) for i in range(num_timestamps)]
    
    # Convert timestamps to numerical values (floats) for plotting
    subsampled_timestamps_numeric = [(ts - min_timestamp).total_seconds() for ts in subsampled_timestamps]
    
    # Interpolate the corresponding states for the subsampled timestamps
    subsampled_states = np.interp(subsampled_timestamps_numeric, [(ts - min_timestamp).total_seconds() for ts in timestamps], numeric_states)
    
    # Plot graph
    fig, ax = plt.subplots(figsize=(10, 6))
    
    # Plot each status change as a vertical line
    for i in range(len(subsampled_timestamps) - 1):
        ax.plot([subsampled_timestamps[i], subsampled_timestamps[i+1]], [subsampled_states[i], subsampled_states[i]], color=cmap[subsampled_states[i]], linewidth=2)
    ...
    plt.ylabel('Status')
    plt.title('Driver Status Over Time (Subsampled Data)')
