...
def graph():
    ...
    # Fetch status information from the database for the last 1 minute
    cursor = get_cursor()
    cursor.execute('''SELECT * FROM driver_status WHERE time >= ? ORDER BY time ASC''', (one_minute_ago,))
    ...
    ax.plot(timestamps, numeric_states, marker='o', linestyle='-', color='#00FFFF', alpha=0.7)  # Adjust color and transparency
    ax.set_xlabel('Time',color='white',fontsize=14)
    ax.set_ylabel('State',color='white',fontsize=14)
    ax.set_title('Driver State Over Time',color='white',fontsize=15)

    # Change the color of the x-axis and y-axis lines to white
    ax.spines['bottom'].set_color('white')  # x-axis
    ax.spines['left'].set_color('white')    # y-axis

    #Change backround color to black
    ...
    ax.set_yticklabels(["noDriver", "sleepy", "drowsy", "active"],color='white',fontsize=11.5)

    # Customize grid lines
    ax.grid(True, linestyle='--', alpha=0.5)

    # Add legend
    ax.legend(['State'], loc='upper right',facecolor='black',edgecolor='white',labelcolor='white')

    # Tighten layout
    plt.tight_layout()

    # Save the plot to a file (optional)
    plt.savefig('static/graph.png',facecolor='black',edgecolor='black',transparent=False)
